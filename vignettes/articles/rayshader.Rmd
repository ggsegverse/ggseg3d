...
```
## Error in `subsceneInfo()`:
## ! No subscene info available.
```

```
## Error in `useSubscene3d()`:
## ! Subscene 0 not found.
```
...
<div class="figure">
<img src="figures/rayshader-first.png" alt="Left hemisphere of the Desikan-Killiany atlas rendered with rgl." width="800" />
...
```
## Error in `rayshader::render_highquality()`:
...
## ! Cannot find the file(s): "figures/rayshader-raytrace.png"
```
...
outfile_lateral <- tempfile(fileext = ".png")
rgl::snapshot3d(outfile_lateral)
rgl::close3d()
```
...
outfile_medial <- tempfile(fileext = ".png")
rgl::snapshot3d(outfile_medial)
rgl::close3d()
```


``` r
p <- ggsegray(atlas = dk, hemisphere = "right") |>
  pan_camera("right superior")

outfile_superior <- tempfile(fileext = ".png")
rgl::snapshot3d(outfile_superior)
rgl::close3d()
```
...
outfile_custom <- tempfile(fileext = ".png")
rgl::snapshot3d(outfile_custom)
rgl::close3d()
```
...
```
## Error in `knitr::include_graphics()`:
## ! Cannot find the file(s): "figures/rayshader-glassbrain.png"
```
...
outgif <- tempfile(fileext = ".gif")
rgl::movie3d(
  rgl::spin3d(axis = c(0, 1, 0), rpm = 10),
  duration = 3,
  dir = dirname(outgif),
  movie = tools::file_path_sans_ext(basename(outgif)),
  type = "gif",
  clean = TRUE
)
rgl::close3d()
