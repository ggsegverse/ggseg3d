...
![Left hemisphere of the Desikan-Killiany atlas rendered with rgl.](img/ray-first.png)
...
`region` column, point `colour_by` at a numeric or categorical variable, and
...
![Data-mapped brain with a custom colour palette in rgl.](img/ray-data.png)
...
![rgl render with a dark background.](img/ray-dark.png)
...
![Subcortical atlas with a translucent glass brain rendered in rgl.](img/ray-glassbrain.png)
...
The `tracula` atlas works with `ggsegray()` too. Tracts render as tube meshes:
...
![White matter tracts with a translucent glass brain in rgl.](img/ray-tracula.png)
...
![Orientation-coloured tracts on a dark background.](img/ray-tracula-orient.png)

## Material properties

`ggsegray()` accepts a `material` argument --- a named list of rgl material
properties (see `?rgl::material3d` for the full list). This gives you control
over lighting, reflection, and shading without ggseg3d needing to wrap every
option.
...
![Glossy surface with specular highlights.](img/ray-glossy.png)
...
![Flat colours with no lighting --- exact colour reproduction for masks.](img/ray-flat.png)
...
![Wireframe view of the mesh geometry.](img/ray-wireframe.png)

Any property accepted by `rgl::material3d()` can be passed via the `material`
list --- `ambient`, `emission`, `smooth`, `alpha`, `lwd`, and more.
