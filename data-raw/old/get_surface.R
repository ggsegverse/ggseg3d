get_surface = function(folder, atlasname) {
  ...
  files = sapply(hemi, list.files, pattern = "*roi*", full.names = T) |>
    as.character()
  ...
  data = data.frame(files = files) |>
    ...
    separate(
      files,
      c("DEL1", "DEL2", "DEL#", "DEL4", "DEL5", "DEL6", "filename"),
      sep = "/"
    ) |>
    select(-contains("DEL")) |>
  ...
  for (i in 1:length(mesh)) {
    data$mesh[[i]] = list(vb = mesh[[i]]$vb, it = mesh[[i]]$it)
  }
