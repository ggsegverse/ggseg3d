...
mesh = lapply(
  list.files(
    "data-raw/mesh3d/lh.cortex.LCBC.fsaverage5/",
    pattern = "ply",
    full.names = T
  ),
  geomorph::read.ply,
...
for (i in 1:length(mesh)) {
  lh$mesh[[i]] = list(vb = mesh[[i]]$vb, it = mesh[[i]]$it)
}

mesh = lapply(
  list.files(
    "data-raw/mesh3d/rh.cortex.LCBC.fsaverage5/",
    pattern = "ply",
    full.names = T
  ),
  geomorph::read.ply,
...
rh$mesh = list(vb = 1)
for (i in 1:length(mesh)) {
  rh$mesh[[i]] = list(vb = mesh[[i]]$vb, it = mesh[[i]]$it)
}
