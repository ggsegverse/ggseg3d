...
aseg_3d = list.files(
  ...
  separate(
    files,
    c("DEL", "DEL1", "DEL2", "DEL3", "DEL4", "roi", "DEL5"),
    remove = F
  ) |>
  select(-contains("DEL")) |>
...
aseg_3d = aseg_3d |>
  left_join(
    read.table(
      "data-raw/mesh3d/aseg/annot2filename.csv",
      sep = "\t",
      header = T,
      stringsAsFactors = F
    ) |>
...
for (i in 1:length(mesh)) {
  aseg_3d$mesh[[i]] = list(vb = mesh[[i]]$vb, it = mesh[[i]]$it)
}

aseg_3d = aseg_3d |>
  mutate(region = label, surf = "LCBC") |>
